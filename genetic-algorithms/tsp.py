...
data = [
    [0,   14.6576,    4.2525,   23.7069,    0.6256,   24.6214,   22.2886,   19.1289,    8.7321,   22.6564],
    [14.6576, 0  ,  8.9398 ,   5.4347   ,20.2467   , 0.1606  , 24.1944 ,  23.5736,   13.2308,   15.1795],
    [4.2525, 8.9398, 0  ,  1.2846 ,  14.3274 ,  24.9323   , 2.7980  ,  4.8370 ,  14.5945 ,  18.9410],
    [23.7069, 5.4347, 1.2846, 0 ,  24.7696  , 22.3167  ,  3.1209 ,  12.6025  ,  7.8043  , 20.0605],
    [0.6256,20.2467,   14.3274 ,  24.7696, 0  , 24.6449 ,   7.7689 ,  19.1153  , 14.4260  ,  7.5117],
    [24.6214 ,   0.1606 ,  24.9323  , 22.3167  , 24.6449 ,     0  ,  3.4621  ,  5.7900   , 2.9587  , 14.0983],
    [22.2886 ,  24.1944 ,   2.7980 ,   3.1209  ,  7.7689 ,   3.4621,   0  , 13.1043 ,   6.6078  , 23.6844],
    [19.1289 ,  23.5736 ,   4.8370  , 12.6025 ,  19.1153  ,  5.7900  , 13.1043  ,       0  ,  0.2621 ,   6.2250],
    [8.7321  , 13.2308  , 14.5945 ,   7.8043   ,14.4260 ,   2.9587  ,  6.6078 ,   0.2621   ,      0  ,  4.8327],
    [22.6564 ,  15.1795  , 18.9410 ,  20.0605 ,   7.5117 ,  14.0983 ,  23.6844  ,  6.2250  ,  4.8327  ,  0]
    ]
pen = 10**3
...
def fitness(individual):
    ...
    #Last evaluation
    if individual[len(individual)-1] == individual[0]:
            total += pen
    else:
        total += data[individual[len(individual)-1]][individual[0]]
    #Check repetitions
    dict2 = {}
...
def main_bin(method):
    ...
    while generation < max_generations+1:
     ...
     #   print(evaluation)
        ...
        print("Indice: ",worst_index)
        print("Maximum fitness = {}".format(min( evaluation )))
        print(population_decoded[worst_index])
        best_fitness_values.append(min( evaluation))
...
if __name__ == "__main__":
    ...
    print("Total:")
    print(len(best))
    print("Valor mínimo con representación binaria", round(min_bin,4))
    print("Valor mínimo con representación real", round(min_real,4))
    print("Valor mínimo con representación entera", round(min_ent,4))
