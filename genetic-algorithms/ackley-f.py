...
def generate_chromosome_real():
    return [ random.uniform(-4,4) for _ in range(n)]
...
def plotFinalResults(data, title, schema,color):
    ...
    plt.legend([l1, l2],['('+ schema[y[0]]+ ')', '('+ schema[z[0]]+ ')'])
    plt.show()
...
def main_ent(method):
    ...
    # First evaluation
    evaluation = [fitness(n) for n in population]
    #print("Primera evaluación:")
    #print(evaluation)
    ...
    while generation < max_generations+1:
        ...
        # Evaluation
        new_evaluation = [fitness(n) for n in new_population]
        print("Nueva evaluación:")
        #print(new_evaluation)
