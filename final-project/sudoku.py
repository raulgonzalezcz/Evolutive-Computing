...
Problem: Solves a Sudoku puzzle using a genetic algorithm.
Date: 11/06/2019
"""
    ...
    def update_fitness(self):
        ...
        for i in range(0, Nd):  # For each column...
            for j in range(0, Nd):  # For each number within the current column...
                ...
                column_count[int(self.values[i][j])-1] += 1  # ...Update list with occurrence of a particular number.
...
class Sudoku(object):
    ...
    def load(self, path):
        ...
        # values = numpy.genfromtxt(path, delimiter=",").reshape((Nd, Nd)).astype(int)
    ...
    def solve(self):
        Population_s = 100  # Population size
        Ne = int(0.02*Population_s)  # Number of elites
        ...
        self.population.seed(Population_s, self.given)
    
        # For up to number of generations...
        while generation < Ng:
            ...
            best_fitness = 0.0
            for c in range(0, Population_s):
                fitness = self.population.candidates[c].fitness
                if(fitness == 1):
                    print("Solution found at generation %d!" % generation)
                    print self.population.candidates[c].values.astype(int)
                    Fitness_val.append(1-fitness)
                    generation = Ng
            ...
            print("Best fitness: %f" % (1 - best_fitness))
            Fitness_val.append(1 - best_fitness)
            ...
            for count in range(Ne, Population_s, 2):
                ...
                ## Cross-over.
                cc = CycleCrossover()
                child1, child2 = cc.crossover(parent1, parent2, crossover_rate=crossover_rate)
        ...
        # Reached maximun number of generations
        if generation == Ng:
            print("No solution found.")
        return self.population.candidates[0], Fitness_val
...
if __name__ == "__main__":
    ...
    Ng = 1000
    s = Sudoku()
    s.load("puzzle_conf2.txt")
    for i in range(iterations):
        solution, Fitness_val = s.solve()
        # Complete data to plot
        last = len(Fitness_val)
        rest = Ng - last
        b = [Fitness_val[last-1] ] * rest
        Fitness_val.extend(b)
        Total_results.append(Fitness_val)
    plotResultsMethod(Ng,Total_results, "GA performance solving Sudoku")
    if(solution):
