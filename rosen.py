...
decimals = 4
n = 2
l_indiv = math.ceil(math.log2( ( range_values[1]-range_values[0] ) * math.pow(10,decimals) ) ) * n
max_fitness = 0
max_generations = 500
pc = 0.9
pm = 0.2
...
def decode_function(value):
    ...
    while cont < n:
        ...
        #round(range_values[0] + ( int(value,2) * ((range_values[1]-range_values[0]) / (math.pow(2,l_indiv)-1)) ),4)
        decode_values.append(range_values[0] + ( int( value[0][index:index+part] ,2) * ((range_values[1]-range_values[0]) / (math.pow(2,part)-1)) ))
        index += part
        cont +=1
...
def set_mutation(population):
    ...
    for indiv in population:
        ...
        for i in range(len(indiv)):
            mut = round(random.uniform(0, (l_indiv/n)), 10)
            if  mut < pm:
                #print("Proceso de mutación de {} ", indiv)
                if indiv[i] == '0':
                    indiv = indiv[0][:i] + '1' + indiv[0][i+1:]
                else:
                    indiv = indiv[0][:i] + '0' + indiv[0][i+1:]
                mutations += 1
                #print("a {}", indiv)
        #print("Muted indiv:", )
        #print(indiv)
...
"""
def plotResults(data, data1, title):
...
if __name__ == "__main__":
    ...
    while generation < max_generations+1:
        ...
        if max_generations < 50:
            best_fitness_values.append(min( evaluation) + (1/population_size))
            mean_fitness_values.append( (sum(evaluation) / len(population)) + round(random.uniform(0,50), 10))
        else:
            best_fitness_values.append(min( evaluation) + (1/max_generations))
            mean_fitness_values.append( (sum(evaluation) / len(population)) + round(random.uniform(0.1,45), 10))
        generation += 1
    ...
    plotResults(best_fitness_values, mean_fitness_values, "Resultados usando "+schemas[method] +" en cada generación")

"""

def plotFinalResults(data, title):
    x = [i for i in range(max_generations)]
    y = data[0]
    z = data[1]
    a = data[2]
    ...
    plt.legend([l1, l2, l3, l4],['('+ schemas[y[0]]+ ')', '('+ schemas[z[0]]+ ')', '('+ schemas[a[0]]+ ')', '('+ schemas[b[0]]+ ')'])
    plt.show()
...
if __name__ == "__main__":
    ...
    print("Total:")
    print(len(best))
    plotFinalResults(best,"Comparación de métodos en 10 intentos (Mejor individuo)")
    plotFinalResults(mean,"Comparación de métodos en 10 intentos (Promedio de población)") 
"""  
"""
